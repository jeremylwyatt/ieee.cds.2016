...
\title{A Multi-Modal Model of Object Deformation for Robotics
...
% The paper headers
\markboth{IEEE CDS}%
{Arriola-Rios and Wyatt: A Multi-Modal Model of Object Deformation for Robotics}
...
\caption{The learning for a specific material comprises two learned models of sensorimotor contingencies. Both are learned from the same data. The first (FP) learns to predicts the force experienced for some motion and material. The second (SP) predicts the deformation of the object given the applied force. The strain $st$ is calculated directly from the interpenetration of the finger into the object. This can be calculated by vision (V) or from the planned motion of the finger.}
\label{fig:learning}
\end{figure}

\comment{The framework consists of three separate systems: a learning system, a prediction system, and a classification system. The learning system is depicted in Figure~\ref{fig:learning}. This enables learning of sensorimotor contingencies from real robot data. Learning of the force prediction (FP) model relies on comparing the predicted $\hat{f}$ and the experienced $f$ forces at the finger tip, as measured by a force-torque sensor (FM). Learning of the shape prediction (SP) model relies on comparing the predicted deformation $\hat{sh}$ with the actual deformation $sh$ measured by visual tracking (V). }

\comment{The two learned models, FP and SP, can be sequenced to create a single, multi-modal predictor (Figure~\ref{fig:prediction}). This combined model takes a planned motion of the finger, and predicts the resulting forces experienced. It then uses both the position and forces to predict the resulting object deformation. Typically a sequence of planned finger positions will be known. In prediction mode the models make predictions based on this sequence. To create the prediction for time $t+1$ the previous predicted state at time $t$ is required: thus the predictions are recursive. This enables predictions over arbitrarily long time periods, but makes learning a good predictor challenging.}
